...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, save_loss_csv=False):
    ...
    # Timing: initialize accumulators and helpers
    def _sync_cuda():
        try:
    ...
    if save_loss_csv:
        ...
        csv_writer.writerow(['iteration', 'total_loss', 'l1_loss', 'ema_loss'])
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            if iteration < opt.iterations:
                ...
                timer_end('optimizer', _t0)

            # Count executed iteration
    ...
    for name in ('render', 'loss', 'densification', 'optimizer'):
        total_t = timers.get(name, 0.0)
        count = counters.get(name, 0)
        avg_per_executed_iter = (total_t / executed_iterations) if executed_iterations > 0 else 0.0
        avg_per_active_iter = (total_t / count) if count > 0 else 0.0
        print(f"- {name}: total {total_t:.6f}s | avg per executed iter {avg_per_executed_iter:.6f}s | avg per active iter {avg_per_active_iter:.6f}s (runs: {count})")
